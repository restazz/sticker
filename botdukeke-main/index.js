            ...
            case 'kangcopet':
				...
				case 's':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(` duração máxima do vídeo é de apenas 9 segundos }
					break
				case 'getses':
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `virou admin, respeita ele pra nao tomar ban :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
					...
					if (mentioned.length > 1) {
						...
						mentions(`foi de base @${mentioned[0].split('@')[0]} fodassi🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
